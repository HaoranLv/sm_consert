...
def load_sts(year, dataset_names, need_label=False, no_pair=False):
    ...
    sts_data_path = f"/opt/ml/input/data/STS/STS{year}-en-test"
...
def load_stsbenchmark(need_label=False, use_all_unsupervised_texts=True, no_pair=False):
    ...
    if use_all_unsupervised_texts:
        ...
        splits = ["test"]
    for split in splits:
        sts_benchmark_data_path = f"/opt/ml/input/data/STS/STSBenchmark/sts-{split}.csv"
...
def load_shulex(need_label=False, use_all_unsupervised_texts=True, no_pair=False):
    if need_label:
        assert not no_pair, "Only paired texts need label"
    logging.info("Loading STSBenchmark dataset")
    all_samples = []
    if use_all_unsupervised_texts:
        ...
        splits = ["test"]
    for split in splits:
        sts_benchmark_data_path = f"/opt/ml/input/data/STS/STSShulex/sts-{split}.csv"
        with open(sts_benchmark_data_path, "r") as f:
            lines = [line.strip() for line in f if line.strip()]
        samples = []
        for line in lines:                
            ...
            if need_label:
                ...
                if no_pair:
                    ...
                    samples.append(InputExample(texts=[sent1, sent2]))
        all_samples.extend(samples)
    logging.info(f"Loaded examples from STSshulex dataset, total {len(all_samples)} examples")
    return all_samples
...
def load_sickr(need_label=False, use_all_unsupervised_texts=True, no_pair=False):
    ...
    for split in splits:
        sick_data_path = f"/opt/ml/input/data/SICK/SICK_{split}.txt"
...
def load_datasets(datasets=None, need_label=False, use_all_unsupervised_texts=True, no_pair=False):
    ...
    datasets = datasets or ["sts12", "sts13", "sts14", "sts15", "sts16", "stsb", "sickr", "shulex"]
    all_samples = []
